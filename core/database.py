...
def validate_database_url(url: str) -> bool:
    ...
    try:
        ...
        if "password" in url and "@localhost" in url and os.getenv("ENVIRONMENT") == "production":
...
def create_database_engine() -> Engine:
    ...
    try:
        ...
        logger.info(f"Database engine created for {parsed.scheme}://{parsed.hostname}:{parsed.port}")
        return engine
    except Exception as e:
        ...
        try:
            with engine.connect() as connection:
                ...
                if test_value and test_value[0] == 1:
                    ...
                    logger.info(f"Database connection test successful ({response_time}ms)")
                    return True
                else:
    ...
    while retry_count < max_retries:
        try:
            ...
            if retry_count >= max_retries:
                logger.error(f"Database session failed after {max_retries} retries: {e}")
                raise DatabaseError(f"Database session failed after retries: {e}")
            else:
                logger.warning(f"Database session retry {retry_count}/{max_retries}: {e}")
